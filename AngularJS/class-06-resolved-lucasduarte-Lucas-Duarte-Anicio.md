...
- 1)Alterar o metodo delete para utilizar a função `splice`
```html
<!doctype html>
...
<body data-ng-controller="UserController as User">
  ...
  <section class="row wrapper">
    ...
    <button data-ng-click="User.remove()" class="btn btn-large red waves-effect waves-light col s12" type="submit" name="action">Remove
    </button>
  </section>
  ...
  <script>
    ...
    function UserController() {
      ...
      function remove(users) {
        vm.users = angular.forEach(vm.users, function(row, index) {
          if(vm.users[index].selecionado) {
